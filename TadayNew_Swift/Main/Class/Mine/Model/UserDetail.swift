...
struct UserDetail: HandyJSON {
    ...
    var descriptionHeight: CGFloat { return Calculate.textHeight(text: description, fontSize: 13, width: SCREEN_WIDTH - 30.0) }
...
struct UserDetailDongtai: HandyJSON {
    ...
    var collectionViewW: CGFloat {
        return isDongtaiDetail ? (SCREEN_WIDTH - 30) : Calculate.collectionViewWidth(thumb_image_list.count)
    }
    ...
    var detailContentH: CGFloat {
        return Calculate.textHeight(text: content, fontSize: 17, width: SCREEN_WIDTH - 30.0)
    }
    
    var contentH: CGFloat {
        let height = Calculate.textHeight(text: content, fontSize: 17, width: SCREEN_WIDTH - 30.0)
        return height >= 110 ? 110 : height
    }
    
    /// 富文本内容高度
    var attributedCntentHeight: CGFloat {
        let height = Calculate.attributedTextHeight(text: attributedContent, width: SCREEN_WIDTH - 30.0)
...
struct DongtaiOriginThread: HandyJSON {
    ...
    var contentH: CGFloat {
        ...
        let height = Calculate.textHeight(text: nameAndContent, fontSize: 17, width: SCREEN_WIDTH - 30.0) + 5.0
        return height >= 120 ? 120 : height
    }
    var attributedContent: NSAttributedString {
        let emojimanager = EmojiManager()
        let mutableAtttributedString = NSMutableAttributedString(string: (user.screen_name == "" ? "" : "\(user.screen_name):"), attributes: [.foregroundColor: ZLBlueFontColor()])
...
struct UserDetailQuestion: HandyJSON {
    ...
    var title: String = ""
    var titleH: CGFloat {
        return Calculate.textHeight(text: title, fontSize: 17, width: SCREEN_WIDTH - 30.0) + 5
    }
...
struct UserDetailAnswerContentAbstract: HandyJSON {
    ...
    var textHeight: CGFloat {
        let height = Calculate.textHeight(text: text, fontSize: 16, width: SCREEN_WIDTH - 30.0)
